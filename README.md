...
Pour l'instant, le projet génère le domaine : réseau de vallées, massifs, pics rocheux, érosion, lacs gelés, torrents, forêts étagées et une aire plate pour la station, avec un cycle jour-nuit. Le joueur peut déjà y construire des maisonnettes.
...
| Commande | Rôle |
|---|---|
| `npm test` | tests du générateur (déterminisme, pentes, drainage, lacs, rivières) et de la météo |
...
src/
  ...
  gen/                    génération : données pures, sans Three.js
    ...
    domain.ts             orchestration : relief, pics, lacs, aire de la station, hydrologie
  ...
  render/                 affichage : Three.js
    ...
    dayCycle.ts           cycle jour-nuit, alpenglow, mer de nuages
...
Les réglages du jeu (tailles de carte, seuils de neige, vent, durée de la journée, zoom…) se modifient dans `src/params/`. `CONTEXT.md` décrit le projet en détail pour reprendre le travail sans historique.
